...
defineOptions({ layout: (h, page) => h(AdminLayout, { title: 'Laporan Finansial', subtitle: 'Dokumen Resmi' }, () => page) })
...
<template>
    ...
    <div class="h-full w-full overflow-y-auto custom-scrollbar pr-2 pb-6 print:overflow-visible print:h-auto">
        <!-- 
        ...
        <div class="flex flex-col sm:flex-row justify-between items-end sm:items-center border-b border-slate-200 pb-3 print-hide">
            <div>
                <h1 class="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                    ...
                    <div class="relative inline-block ml-1">
                        <select :value="year" @change="changeYear" class="appearance-none bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold py-1 pl-3 pr-8 rounded-lg cursor-pointer outline-none transition-colors border-none focus:ring-0">
                            <option v-for="y in availableYears" :key="y" :value="y">{{ y }}</option>
                        </select>
                        <ChevronDown class="h-3 w-3 absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" />
                    </div>
                </h1>
                <p class="text-[12px] text-slate-500 mt-1 font-medium">Dokumen rekapitulasi transaksi resmi dan performa finansial.</p>
            </div>

            <!-- Tombol Cetak yang Ramping dan Minimalis (Subtle) -->
            <button @click="printReport" class="mt-4 sm:mt-0 inline-flex items-center gap-2 px-3 py-1.5 text-[12px] font-bold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-blue-600 transition-colors shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
        ...
        <div class="hidden print-block text-center border-b-[3px] pb-5 mb-6 border-slate-900">
            <h1 class="text-2xl font-black uppercase tracking-widest text-slate-900">GearHike - Laporan Laba Bersih</h1>
            <p class="text-sm text-slate-600 font-semibold mt-1">Periode Transaksi Laporan: Tahun {{ year }}</p>
        </div>
        ...
        -->
        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.04)] leading-relaxed text-[13px] text-slate-600 print-no-border print-no-padding">
            <p>
                Berdasarkan rekapitulasi data finansial yang terekam pada sistem, selama periode <strong>Tahun {{ year }}</strong>, 
                perusahaan telah berhasil memproses total <strong>{{ stats.total_rentals_year }} transaksi penyewaan</strong> yang selesai secara penuh. 
                Secara agregat, volume transaksi ini mengkontribusikan pendapatan kotor sebesar <strong class="text-slate-900 tabular-nums font-black bg-slate-100 px-1 rounded">{{ formatCurrency(stats.total_income_year) }}</strong>. 
                Bila dianalisa lebih lanjut, rata-rata pendapatan yang dihasilkan per transaksi (AOV) pada rentang tahun ini terukur di angka <strong class="text-slate-900 tabular-nums font-bold">{{ formatAOV(stats.total_income_year, stats.total_rentals_year) }}</strong>.
                Untuk performa khusus di bulan berjalan ini, telah terkumpul <strong>{{ stats.current_month_rentals }} transaksi</strong> senilai <strong class="tabular-nums font-bold text-slate-800">{{ formatCurrency(stats.current_month_income) }}</strong>.
            </p>
        </div>
        ...
        -->
        <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-[0_2px_8px_-3px_rgba(0,0,0,0.04)] print-no-border">
            <table class="w-full text-left text-[12px] whitespace-nowrap">
                <thead class="bg-slate-50/80 border-b border-slate-200 text-slate-500 font-extrabold text-[10px] uppercase tracking-widest">
                    <tr>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="(row, index) in monthlyReport" :key="row.month" 
                        class="group hover:bg-slate-100/50 even:bg-slate-50/80 transition-colors">
                        <!-- Nama Bulan -->
                        <td class="px-5 py-2.5 font-bold text-slate-700">{{ row.month_name }}</td>
                        
                        <!-- Volume Transaksi -->
                        <td class="px-5 py-2.5 text-center text-slate-600 tabular-nums font-medium">
                            {{ row.rentals > 0 ? row.rentals : '-' }}
                        </td>
                        
                        <!-- Rata-rata per Transaksi (AOV) -->
                        <td class="px-5 py-2.5 text-right text-slate-500 tabular-nums font-medium">
                            {{ row.rentals > 0 ? formatAOV(row.income, row.rentals) : '-' }}
                        </td>
                        
                        <!-- Total Pendapatan Bulan Tersebut -->
                        <td class="px-5 py-2.5 text-right font-black text-slate-900 tabular-nums">
                    ...
                    <tr class="bg-slate-800 text-white font-black">
                        <td class="px-5 py-3.5 uppercase text-[10px] tracking-widest text-slate-300">Total Tahun {{ year }}</td>
                        <td class="px-5 py-3.5 text-center tabular-nums">{{ stats.total_rentals_year }}</td>
                        <td class="px-5 py-3.5 text-right tabular-nums text-slate-300 font-bold">
                            {{ formatAOV(stats.total_income_year, stats.total_rentals_year) }}
                        </td>
                        <td class="px-5 py-3.5 text-right tabular-nums text-[13px] text-white">{{ formatCurrency(stats.total_income_year) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-8 pt-4 flex justify-end print:mt-4 print:pt-2">
            <div class="text-center w-56">
                ...
                <p class="text-[12px] text-slate-600 mb-16 print:mb-12 font-medium uppercase tracking-wider">Mengetahui,</p>
                <div class="border-b-2 border-slate-800 mb-1.5"></div>
                <p class="font-black text-slate-900 text-[13px] tracking-widest uppercase">Administrator</p>
                <p class="text-[10px] text-slate-400 mt-1">Dicetak pada: {{ new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) }}</p>
            </div>
        </div>
    </div>
    </div>
