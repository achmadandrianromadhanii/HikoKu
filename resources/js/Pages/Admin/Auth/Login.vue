    ...
    <div class="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950">
        ...
        <div class="absolute inset-0 z-0">
            <img src="https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=2070&auto=format&fit=crop" 
                ...
                alt="Mountain Background" loading="lazy" />
        ...
        <div class="relative z-10 w-full max-w-[400px] p-6 animate-[fadeInUp_0.8s_ease-out]">
            ...
            >
                <div v-if="page.props.flash?.error" class="mb-4 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-center text-sm font-medium text-red-300 backdrop-blur-md">
                    {{ page.props.flash.error }}
                </div>
            </transition>

            <div class="rounded-[24px] border border-white/10 bg-slate-900/70 p-8 shadow-2xl backdrop-blur-xl">
                ...
                <form class="space-y-8" @submit.prevent="submit">
                    ...
                    <div class="flex items-end gap-3">
                        ...
                        <div class="relative w-full">
                            ...
                            <input v-model="form.email" type="text" id="username" autocomplete="username" placeholder=" "
                    ...
                    <p v-if="form.errors.email" class="ml-9 mt-1 text-xs text-red-400">{{ form.errors.email }}</p>
                    ...
                    <div class="flex items-end gap-3">
                        ...
                        <div class="relative w-full">
                            ...
                            <!-- Toggle Lihat Sandi -->
                            <button type="button" class="absolute bottom-2.5 right-0 text-slate-400 transition-colors hover:text-cyan-400 focus:outline-none" @click="showPassword = !showPassword">
                                <EyeOff v-if="showPassword" class="h-5 w-5" />
                                <Eye v-else class="h-5 w-5" />
                            </button>
                        </div>
                    </div>
                    <p v-if="form.errors.password" class="ml-9 mt-1 text-xs text-red-400">{{ form.errors.password }}</p>

                    <!-- Ingat Sesi -->
                    <div class="flex items-center pt-2">
                        <label class="flex cursor-pointer items-center gap-2 text-sm text-slate-400 transition-colors hover:text-white">
                            <input v-model="form.remember" type="checkbox" class="rounded border-slate-600 bg-slate-800/50 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-slate-900" />
                            Ingat Sesi
                        </label>
                    </div>
