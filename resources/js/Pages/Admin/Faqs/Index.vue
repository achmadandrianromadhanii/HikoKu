...
defineOptions({ layout: (h, page) => h(AdminLayout, { title: 'FAQs', subtitle: 'Kelola pertanyaan umum untuk halaman user dengan tampilan rapi dan compact.' }, () => page) })
...
<template>
    <!-- Persistent AdminLayout -->
        <div class="h-full flex flex-col space-y-4 sm:space-y-6">
            <AdminPageHeader title="Kelola FAQs"
            ...
            <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 shrink-0">
                <AdminStatCard v-for="card in statCards" :key="card.label" :label="card.label" :value="card.value"
                    :icon="card.icon" :variant="card.variant" />
            </section>

            <AdminTableToolbar v-model:search="search" search-placeholder="Cari pertanyaan atau jawaban..."
            ...
            <section class="flex-1 flex flex-col min-h-0 space-y-4">
                <DataTable :columns="columns" :rows="rows" empty-title="Belum ada FAQ"
        ...
        <AppModal :open="openCreateModal" title="Tambah FAQ" max-width="max-w-3xl" @close="openCreateModal = false">
            <div class="space-y-5">
                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-900">Pertanyaan</label>
                    <input v-model="createForm.question" type="text"
                ...
                <div>
                    ...
                    <textarea v-model="createForm.answer" rows="6"
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-slate-900">Sort Order</label>
        ...
        <AppModal :open="openEditModal" title="Edit FAQ" max-width="max-w-3xl" @close="openEditModal = false">
            <div class="space-y-5">
                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-900">Pertanyaan</label>
                    <input v-model="editForm.question" type="text"
                        class="h-11 w-full rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100" />
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-900">Jawaban</label>
                    <textarea v-model="editForm.answer" rows="6"
                        class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100" />
                </div>
            ...
            <template #footer>
                <div class="flex flex-wrap justify-end gap-3">
                    ...
                    <button type="button"
                        class="inline-flex h-10 items-center rounded-2xl bg-emerald-600 px-4 text-sm font-semibold text-white transition hover:bg-emerald-700"
                        :disabled="editForm.processing" @click="submitEdit">
        ...
        <ConfirmModal :open="openDeleteModal" title="Hapus FAQ" message="Yakin ingin menghapus FAQ ini?"
