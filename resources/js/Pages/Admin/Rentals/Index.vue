...
// --- Persistent Layout ---
defineOptions({ layout: (h, page) => h(AdminLayout, { title: 'Rentals', subtitle: 'Pusat operasional rental: payment, jaminan, activate, dan return.' }, () => page) })
...
const processScan = async (code) => {
    ...
    try {
        ...
        if (error.response?.data?.message) {
            uiStore.showToast({ title: 'Tiket Ditolak', message: error.response.data.message, type: 'error' })
        } else {
            uiStore.showToast({ title: 'Gagal Scan', message: 'Terjadi kesalahan jaringan/server saat memproses tiket.', type: 'error' })
        }
    } finally {
...
const tabs = computed(() => [
    ...
    { value: 'pending_payment', label: 'Pending', count: props.stats.pending_payment, color: 'text-amber-600 bg-amber-50' },
    { value: 'confirmed', label: 'Confirmed', count: props.stats.confirmed, color: 'text-blue-600 bg-blue-50' },
    { value: 'active', label: 'Active', count: props.stats.active, color: 'text-emerald-600 bg-emerald-50' },
    { value: 'overdue', label: 'Overdue', count: props.stats.overdue, color: 'text-red-600 bg-red-50' },
])
...
const closeDropdown = (e) => {
    ...
    const isInteractive = ['INPUT', 'TEXTAREA', 'BUTTON', 'A'].includes(tag) || e.target.closest('button') || e.target.closest('a')
...
// --- Actions ---
const openConfirm = (rental) => { selectedRental.value = rental; openConfirmModal.value = true }
const openReturn = (rental) => { selectedRental.value = rental; openReturnModal.value = true }
const openDisputed = (rental) => { selectedRental.value = rental; openDisputedModal.value = true }
...
<template>
    ...
    <div class="h-full flex flex-col space-y-4 sm:space-y-6">
        <div class="flex justify-between items-center shrink-0">
            <AdminPageHeader title="Kelola Rentals" subtitle="Pusat operasional rental: E-Ticket, Scanner, dan Return." />
            
            <div class="flex flex-col items-end gap-1 w-full max-w-xs">
                <div class="relative w-full">
                    <ScanLine class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-cyan-600" />
                    ...
                    <input type="text" ref="barcodeInputRef" :value="scanInput" @input="onScannerInput" @keydown.enter.prevent="handleManualScan" placeholder="Klik disini lalu Scan Barcode..." class="w-full h-9 pl-9 pr-3 text-xs border-cyan-200 rounded-xl focus:border-cyan-500 focus:ring-cyan-500/20 bg-cyan-50/30 placeholder:text-cyan-600/50 text-cyan-900 font-semibold shadow-inner" />
                </div>
                <div v-if="isScanning" class="flex items-center gap-1.5 text-xs font-semibold text-cyan-600 animate-pulse">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-3 rounded-2xl shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] border border-slate-100 shrink-0">
            ...
            <div class="relative w-full sm:w-64" ref="searchInputRef">
                <div class="relative flex items-center">
                    ...
                    <input v-model="search" type="text" placeholder="Ketik 1 huruf untuk mencari..."
                    ...
                    <button v-if="search" @click="clearSearch" class="absolute right-2 p-1 rounded-full hover:bg-slate-200 text-slate-400 transition">
                        <X class="h-3 w-3" />
                    </button>
                </div>
                ...
                <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
                    <div v-if="isSearchFocused && search.length > 0" 
                        ...
                        <div class="max-h-64 overflow-y-auto p-1">
                            ...
                            <Link v-for="row in rows.slice(0,5)" :key="row.id" :href="route('admin.rentals.show', row.id)"
                                class="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-xl transition-colors group">
                                <div class="h-8 w-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0">
                                    <Search class="h-3.5 w-3.5" />
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="text-xs font-semibold text-slate-900 truncate">{{ row.rental_code }}</p>
                                    <p class="text-[10px] text-slate-500 truncate">{{ row.user_name || '-' }}</p>
                                </div>
                                <ChevronDown class="h-4 w-4 text-slate-300 -rotate-90 group-hover:text-blue-500 transition-colors" />
                            </Link>
                        </div>
                        <div class="p-2 border-t border-slate-50 text-center" v-if="rows.length > 5">
                            <span class="text-[10px] text-slate-400">Tekan Enter untuk melihat semua {{ rows.length }} hasil.</span>
                        </div>
                    </div>
                </transition>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] border border-slate-100 flex flex-col min-h-0 flex-1">
            <!-- [UPDATE]: Area scrollbar difokuskan di dalam tabel dengan animasi custom-scrollbar -->
            <div class="flex-1 overflow-auto custom-scrollbar">
                <table class="w-full text-left text-xs whitespace-nowrap">
                    <thead class="bg-slate-50/80 border-b border-slate-100 text-slate-500 font-medium">
                        <tr>
                        ...
                        <tr v-if="rows.length === 0">
                            <td colspan="5" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-slate-400">
                                    ...
                                    <p class="text-sm font-medium text-slate-600">Belum ada rental ditemukan</p>
                                    <p class="text-xs mt-1">Coba sesuaikan filter atau kata kunci pencarian Anda.</p>
                                </div>
                            </td>
                        </tr>
                        
                        <tr v-for="row in rows" :key="row.id" class="group hover:bg-slate-100/50 even:bg-slate-50/80 transition-colors">
                            <!-- Customer Info -->
                            <td class="px-4 py-2.5 pl-5">
                                <div class="flex items-center gap-3">
                                    <div class="h-8 w-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 flex-shrink-0 overflow-hidden">
                                        <UserCircle2 class="h-5 w-5" />
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-bold text-slate-900 truncate">{{ row.user_name || 'No Name' }}</p>
                                        <p class="text-[10px] text-slate-500 truncate mt-0.5">{{ row.user_email || '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-2.5">
                                <Link :href="route('admin.rentals.show', row.id)" class="inline-flex font-bold text-blue-600 hover:text-blue-800 transition hover:underline">
                                    {{ row.rental_code }}
                                </Link>
                                <p class="text-[10px] text-slate-500 mt-0.5 max-w-[150px] truncate" title="Data Produk/Paket bisa ditambahkan disini">
                                    {{ row.items_count ? `${row.items_count} Items` : 'Paket Rental' }}
                                </p>
                            </td>
                            ...
                            <td class="px-4 py-2.5">
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-slate-900">{{ row.grand_total_label || '-' }}</span>
                                    ...
                                    -->
                                    <AppBadge v-if="!(row.status === 'pending_payment' && row.payment_status === 'pending')" 
                                        :variant="paymentVariant(row.payment_status)" class="scale-90 origin-left">
                                        {{ row.payment_status || '-' }}
                                    </AppBadge>
                                    <AppBadge :variant="statusVariant(row.status)" class="scale-90 origin-left">
                            ...
                            <td class="px-4 py-2.5 text-right pr-5">
                                ...
                                -->
                                <div class="flex items-center justify-end gap-1.5 transition-opacity duration-200">
                                    <Link :href="route('admin.rentals.show', row.id)">
                                        <button class="h-7 w-7 rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 flex items-center justify-center transition" title="Detail">
                                    ...
                                    <button v-if="['active', 'overdue'].includes(row.status)" 
                                        @click="openReturn(row)" class="h-7 w-7 rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-amber-600 hover:border-amber-200 hover:bg-amber-50 flex items-center justify-center transition" title="Return Rental">
                                        <RotateCcw class="h-3.5 w-3.5" />
                                    </button>
                                    
                                    <button v-if="['active', 'overdue'].includes(row.status)" 
                                        @click="openDisputed(row)" class="h-7 w-7 rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-red-600 hover:border-red-200 hover:bg-red-50 flex items-center justify-center transition" title="Tandai Bermasalah (Rusak/Hilang)">
        ...
        <!-- Modals -->
        <ConfirmModal :open="openConfirmModal" title="Konfirmasi Rental Manual" message="Gunakan ini hanya untuk alur manual / cash." confirm-text="Ya, Confirm" :loading="actionLoading" variant="primary" @close="openConfirmModal = false" @confirm="executeAction('admin.rentals.confirm', openConfirmModal)" />
        <ConfirmModal :open="openReturnModal" title="Proses Return" message="Pastikan semua barang sudah dicek fisiknya sebelum menyelesaikan return." confirm-text="Ya, Return" :loading="actionLoading" variant="success" @close="openReturnModal = false" @confirm="executeAction('admin.rentals.return', openReturnModal)" />
        
        <ConfirmModal :open="openDisputedModal" title="Tandai Bermasalah" message="Barang rusak atau cacat? Status akan menjadi Disputed dan KTP pelanggan harus Anda tahan sampai ada ganti rugi." confirm-text="Tandai Bermasalah" :loading="actionLoading" variant="danger" @close="openDisputedModal = false" @confirm="executeAction('admin.rentals.dispute', openDisputedModal)" />
