<script setup>
        ...
        layout: (h, page) =>
            h(
                AdminLayout,
                {
                    title: 'Detail Rental',
                    subtitle:
                        'Manajemen komprehensif untuk operasional rental, jaminan, dan pembayaran.',
                },
                () => page
            ),
    });
    ...
    const handleKeydown = (e) => {
        ...
        if (
            openLateFeeModal.value ||
            openConfirmModal.value ||
            openActivateModal.value ||
    ...
    const processScan = async (code) => {
        ...
        if (code !== props.rental.rental_code) {
            uiStore.showToast({
                title: 'Tiket Berbeda',
                message: `Barcode yang discan (${code}) bukan milik rental ini.`,
                type: 'warning',
            });
            return;
        }
        ...
        try {
            ...
            if (data.action_status === 'activated') {
                uiStore.showToast({
                    title: 'Pesanan Masuk',
                    message: 'Sukses! Status otomatis menjadi Aktif.',
                    type: 'success',
                });
                ...
                uiStore.showToast({
                    ...
                    message: 'Stok barang telah dikembalikan otomatis.',
                    type: 'success',
                });
                router.reload({ only: ['rental'], preserveScroll: true });
    ...
    const processReturn = () => {
        ...
        returnForm.post(route('admin.rentals.return', props.rental.id), {
            preserveScroll: true,
            onFinish: () => {
                actionLoading.value = false;
...
<template>
    ...
    <div class="h-full w-full overflow-y-auto custom-scrollbar pr-2 pb-6">
        <div class="space-y-6">
            ...
            -->
                <section
                    class="relative overflow-hidden rounded-2xl bg-white border border-slate-200 p-5 shadow-sm"
                >
                    <div
                        class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-cyan-500 to-blue-600"
                    ></div>
                    ...
                    <div
                        class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between pr-10"
                    >
                        <div>
        ...
        -->
            <div class="grid gap-6 lg:grid-cols-[7fr_3fr] items-start">
                ...
                <div class="space-y-6">
                    ...
                    <div
                        class="animate-fade-up opacity-0 [animation-fill-mode:forwards] [animation-delay:200ms]"
                    >
                        <section
                            class="rounded-xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:border-cyan-100 overflow-hidden"
                        >
                            <div
                            ...
                            <div
                                class="p-4 grid gap-5 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-100"
                            >
                                <!-- Data Penyewa -->
                                ...
                                <div class="lg:pl-5 pt-5 lg:pt-0">
                                    <div class="flex items-center gap-2 mb-3">
                                        ...
                                        >
                                            Item Disewa ({{ rental.items?.length || 0 }})
                                        </h4>
                                    </div>
                                    <div class="space-y-2 max-h-40 overflow-y-auto pr-1">
                                        <div
                                            ...
                                            :key="item.id"
                                            class="flex justify-between items-center bg-slate-50 rounded-lg p-2.5 transition-colors hover:bg-slate-100"
                                        >
                                            <div class="min-w-0 flex-1">
                                                <p
                                                    class="truncate text-[11px] font-semibold text-slate-900"
                                                >
                                                    {{ item.product_name || '-' }}
                                                </p>
                                                <p
            ...
            -->
                <div class="space-y-6 sticky top-6">
                    ...
                    <div
                        class="animate-fade-up opacity-0 [animation-fill-mode:forwards] [animation-delay:300ms]"
                    >
                        <!-- [UPDATE]: Mengurangi padding (p-4) dan tinggi tombol (h-8) 
                         serta font-size untuk menghasilkan panel "Pusat Kendali" ultra-compact -->
                        <section
                            class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm flex flex-col gap-2 transition-all duration-300 hover:shadow-md hover:border-cyan-100 relative overflow-hidden"
                        >
                            <div
                                v-if="isScanning"
                            ...
                            >
                                ...
                                <div class="w-full">
                                    <p class="text-[10px] font-bold text-cyan-800 mb-1">
                                        Scan Barcode Disini
                                    </p>
                                    <input
                                        type="text"
                    ...
                    <div
                        class="animate-fade-up opacity-0 [animation-fill-mode:forwards] [animation-delay:400ms]"
                    >
                        ...
                        <section
                            class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-300 hover:shadow-md hover:border-cyan-100"
                        >
                            <div
                                        ...
                                        <p class="text-[10px] font-bold text-slate-900 truncate">
                                            {{
                                                rental.payment.status === 'paid'
                                                    ? formatDateTime(rental.payment.paid_at)
                                                    : '-'
                                            }}
                                        </p>
                                    </div>
            ...
            >
                <div class="p-6">
                    ...
                    <p class="text-xs font-medium text-slate-500 mb-6">
                        Isi kondisi barang saat dikembalikan. Denda keterlambatan dihitung otomatis
                    ...
                    <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        <div
                            v-for="item in rental.items"
                            :key="item.id"
                            class="p-4 border border-slate-200 rounded-2xl bg-white shadow-sm"
                        >
                            <div
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
                                <div class="space-y-1.5">
                                    ...
                                    >
                                    <select
                                        v-model="returnForm.items[item.id].condition"
                                ...
                                <div class="space-y-1.5">
                                    <label
                                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-500"
                                        >Total Denda (Rp)</label
                                    >
                                    <input
                                        type="number"
                                        v-model="returnForm.items[item.id].penalty"
                                        min="0"
                                        class="h-9 w-full text-sm font-medium border-slate-200 rounded-xl focus:border-cyan-500 focus:ring-cyan-500/20"
                                    />
                                </div>
                                ...
                                <div
                                    v-if="['missing'].includes(returnForm.items[item.id].condition)"
                                    class="space-y-1.5"
                                >
                                    <label
                                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-500"
                                    ...
                                    <input
                                        type="number"
                                        v-model="returnForm.items[item.id].missing_qty"
                                        min="0"
                                        :max="item.quantity"
                                        class="h-9 w-full text-sm font-medium border-slate-200 rounded-xl focus:border-cyan-500 focus:ring-cyan-500/20"
                                    />
                                ...
                                <div
                                    v-if="['damaged'].includes(returnForm.items[item.id].condition)"
                                    class="space-y-1.5"
                                >
                                    <label
                                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-500"
                                    ...
                                    <input
                                        type="number"
                                        v-model="returnForm.items[item.id].damaged_qty"
                                        min="0"
                                        :max="item.quantity"
                                        class="h-9 w-full text-sm font-medium border-slate-200 rounded-xl focus:border-cyan-500 focus:ring-cyan-500/20"
                                    />
                                ...
                                <div class="sm:col-span-2 space-y-1.5">
                                    ...
                                    >
                                    <textarea
                                        v-model="returnForm.items[item.id].notes"
                                        rows="2"
                                        class="w-full text-sm font-medium border-slate-200 rounded-xl focus:border-cyan-500 focus:ring-cyan-500/20"
