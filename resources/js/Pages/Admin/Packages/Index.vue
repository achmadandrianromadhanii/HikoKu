...
defineOptions({ layout: (h, page) => h(AdminLayout, { title: 'Packages', subtitle: 'Kelola paket bundling rental.' }, () => page) })
    ...
    <div class="flex flex-col h-full gap-4">
            ...
            <div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                <div>
                    ...
                    <p class="mt-1 text-xs font-medium text-slate-500">Koleksi paket bundling premium untuk user.</p>
                </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-100">
                <!-- Search Input Mungil -->
                <div class="relative w-full max-w-xs">
                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        <Search class="h-4 w-4 text-slate-400" />
                    </div>
                    <input v-model="search" type="text" placeholder="Ketik 1 huruf..."
                ...
                <label class="group flex cursor-pointer items-center gap-2">
                    <div class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors" :class="activeOnly ? 'bg-cyan-500' : 'bg-slate-200'">
                        ...
                        <span class="inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform" :class="activeOnly ? 'translate-x-4' : 'translate-x-1'"></span>
                    </div>
                    <span class="text-[11px] font-bold uppercase tracking-wider text-slate-500 transition-colors group-hover:text-slate-700">Aktif Saja</span>
                </label>
            </div>

            <!-- Tabel Data Level Dewa -->
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-100">
                <div class="flex-1 min-h-0 overflow-auto custom-scrollbar relative z-10">
                    <table class="w-full text-left text-[12px] text-slate-600 whitespace-nowrap">
                        <thead class="sticky top-0 z-20 border-b border-slate-100 bg-slate-50 text-[10px] font-bold uppercase tracking-widest text-slate-400 shadow-[0_1px_2px_rgba(0,0,0,0.05)] after:absolute after:inset-x-0 after:bottom-0 after:border-b after:border-slate-100/60">
                            <tr>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr v-for="row in rows" :key="row.id" class="group transition-colors hover:bg-slate-100/50 even:bg-slate-50/80">
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3.5">
                                        ...
                                        -->
                                        <div class="relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_2px_8px_-3px_rgba(0,0,0,0.1)]">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <span class="font-bold text-slate-900 transition-colors group-hover:text-cyan-600">{{ row.name }}</span>
                                                <Star v-if="row.is_featured" class="h-3 w-3 fill-amber-400 text-amber-400" />
                                            </div>
                                            <span class="mt-0.5 block text-[10px] text-slate-400">{{ row.slug }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex max-w-xs flex-wrap gap-1.5">
                                        <span v-for="item in row.items" :key="item.product_id"
                                            class="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600">
                                            {{ item.product?.name }}
                                            <span class="font-bold text-slate-400">x{{ item.quantity }}</span>
                                        </span>
                                    </div>
                                </td>
                                
                                <td class="px-4 py-3 text-right">
                                    <span class="block font-bold text-cyan-600">{{ row.price_label }}</span>
                                </td>
                                
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex items-center justify-center rounded-full px-2 py-0.5 text-[10px] font-bold"
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                        <Link :href="route('admin.packages.edit', row.id)"
        ...
        <ConfirmModal :open="openDeleteModal" title="Hapus Paket" message="Yakin ingin menghapus paket bundling ini? Data tidak bisa dikembalikan."
