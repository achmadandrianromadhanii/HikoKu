<script setup>
        ...
        layout: (h, page) =>
            h(
                AdminLayout,
                {
                    ...
                    subtitle: 'Buat paket bundling untuk katalog user dan checkout.',
                },
                () => page
            ),
    });
    ...
    // State untuk menampilkan/menyembunyikan form deskripsi (Progressive Disclosure)
    const showDescription = ref(false);

    // Form state yang terhubung dengan Inertia
    const form = useForm({
        name: '',
        description: '',
        price_per_day: '',
    ...
    // Menyimpan preview URL sementara saat user memilih foto
    const imagePreview = ref(null);

    // Menangani event saat file gambar dipilih
    ...
    // Menambah baris produk baru ke dalam paket
    const addItem = () => {
        form.items.push({
            product_id: '',
            quantity: 1,
    ...
    // Menghapus baris produk dari paket
    ...
    // Proses submit form menggunakan multipart/form-data untuk gambar
    const submit = () => {
...
<template>
    ...
    <div class="mx-auto max-w-5xl space-y-4">
        <!-- Header Ringkas -->
        <div
            class="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100"
        >
            <div>
                <h1 class="text-xl font-black text-slate-900">Tambah Paket Bundling</h1>
                <p class="mt-0.5 text-[11px] font-medium text-slate-500">
                    Kumpulkan produk jadi satu paket hemat.
                </p>
            </div>
            <Link prefetch
        ...
        <form class="grid gap-4 lg:grid-cols-12" @submit.prevent="submit">
            ...
            <div class="space-y-4 lg:col-span-7">
                ...
                <section class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label
                                class="mb-1.5 block text-[11px] font-bold uppercase tracking-wider text-slate-500"
                                >Nama Paket</label
                            >
                            <input
                                v-model="form.name"
                        ...
                        <div>
                            <label class="group flex cursor-pointer items-center gap-2">
                                ...
                                <span
                                    class="text-[11px] font-bold text-slate-600 transition-colors group-hover:text-slate-900"
                                    >Tambahkan Deskripsi Detail?</span
                                >
                            </label>

                            <!-- Animasi v-show untuk textarea deskripsi agar UI tidak penuh jika tak perlu -->
                            <div v-show="showDescription" class="mt-3">
                                <textarea
                                    ...
                                    placeholder="Tuliskan spesifikasi, syarat, atau fasilitas paket..."
                                ></textarea>
                                <p
                ...
                <section class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
                    <div
                        class="mb-4 flex items-center justify-between border-b border-slate-50 pb-3"
                    >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            "
                                        >
                                            <span class="truncate text-[12px] font-semibold">{{
                                                product.name
                                            }}</span>
                    ...
                    <p v-if="form.errors.items" class="mt-2 text-[10px] text-rose-500">
                        {{ form.errors.items }}
                    </p>
                </section>
            </div>
            ...
            <div class="space-y-4 lg:col-span-5">
                ...
                <section class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
                    ...
                    <div
                        class="relative flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-4 transition-colors hover:border-purple-400 hover:bg-purple-50/50"
                    >
                        <!-- Preview Image Jika Ada -->
                        <div
                            v-if="imagePreview"
                            class="relative mb-3 h-32 w-32 overflow-hidden rounded-lg shadow-sm"
                        >
                            <img
                                :src="imagePreview"
                                alt="Preview"
                        ...
                        <div
                            v-else
                            class="mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-sm"
                        >
                    ...
                    <p v-if="form.errors.image" class="mt-2 text-center text-[10px] text-rose-500">
                        {{ form.errors.image }}
                    </p>
                </section>
                ...
                <section class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
                    <div>
                        ...
                        <p v-if="form.errors.price_per_day" class="mt-1 text-[10px] text-rose-500">
                ...
                <section class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
                    <div class="mb-4 flex items-center gap-3">
                        <label class="group flex cursor-pointer items-center gap-2">
                            ...
                            >
                                ...
                                <span
                                    class="inline-block h-3 w-3 transform rounded-full bg-white shadow-sm transition-transform"
                                    :class="form.is_active ? 'translate-x-3.5' : 'translate-x-0.5'"
                                ></span>
                            </div>
                            <span
                                class="text-[11px] font-bold text-slate-600 transition-colors group-hover:text-slate-900"
                        ...
                        <label class="group flex cursor-pointer items-center gap-2">
                            <div
                                class="relative inline-flex h-4 w-7 items-center rounded-full transition-colors"
                                :class="form.is_featured ? 'bg-amber-400' : 'bg-slate-200'"
                            >
                                <input type="checkbox" v-model="form.is_featured" class="sr-only" />
                                <span
                                    class="inline-block h-3 w-3 transform rounded-full bg-white shadow-sm transition-transform"
                                    :class="
                                        form.is_featured ? 'translate-x-3.5' : 'translate-x-0.5'
                                    "
                                ></span>
                            </div>
                            <span
                    ...
                    <div
                        class="mb-4 flex items-center justify-between rounded-lg bg-slate-50 p-2 ring-1 ring-slate-100/50"
                    >
                        <span class="text-[11px] font-bold text-slate-500">Total Item</span>
                        <span
                            class="flex h-6 w-6 items-center justify-center rounded-md bg-white text-[11px] font-black text-slate-900 shadow-sm"
                            >{{ totalItemCount }}</span
                        >
                    </div>
                    ...
                    >
                        <Save class="h-4 w-4" />
                        {{ form.processing ? 'Menyimpan...' : 'Simpan Paket Bundling' }}
                    </button>
                </section>
            </div>
        </form>
