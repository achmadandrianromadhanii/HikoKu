...
// --- Persistent Layout ---
defineOptions({ layout: (h, page) => h(AdminLayout, { title: 'Payments', subtitle: 'Monitoring pembayaran transaksi secara real-time.' }, () => page) })
...
const tabs = computed(() => [
    ...
    { value: 'pending', label: 'Pending', count: props.stats.pending, color: 'text-amber-600 bg-amber-50' },
    { value: 'paid', label: 'Paid', count: props.stats.paid, color: 'text-emerald-600 bg-emerald-50' },
    { value: 'failed', label: 'Failed', count: props.stats.failed, color: 'text-red-600 bg-red-50' },
    { value: 'refunded', label: 'Refunded', count: props.stats.refunded, color: 'text-blue-600 bg-blue-50' },
])
...
const getMethodIcon = (value) => {
    ...
    if (val.includes('transfer') || val.includes('bank') || val.includes('bca') || val.includes('mandiri')) return CreditCard
    return Wallet
}
...
<template>
    <div class="h-full flex flex-col space-y-4 sm:space-y-6">
        <AdminPageHeader title="Kelola Payments" subtitle="Monitoring arus kas dan detail pembayaran dari seluruh pengguna." />
                            ...
                            <div class="max-h-64 overflow-y-auto p-1">
                                ...
                                <div v-for="row in rows.slice(0,5)" :key="row.id"
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-xs font-semibold text-slate-900 truncate">{{ row.payment_code }}</p>
                        ...
                        <tr v-if="rows.length === 0">
                            <td colspan="4" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-slate-400">
                                    ...
                                    <p class="text-sm font-medium text-slate-600">Belum ada pembayaran ditemukan</p>
                        ...
                        <tr v-for="row in rows" :key="row.id" class="group hover:bg-slate-100/50 even:bg-slate-50/80 transition-colors">
                            ...
                            <td class="px-4 py-2.5 pl-5">
                                <div class="flex items-center gap-2 group/copy">
                                    <span class="font-bold text-slate-900 font-mono">{{ row.payment_code || '-' }}</span>
                                    <button v-if="row.payment_code" @click="copyToClipboard(row.payment_code)" 
                                        class="text-slate-300 hover:text-blue-500 transition opacity-0 group-hover/copy:opacity-100 focus:opacity-100" title="Copy Payment Code">
                                        <Check v-if="copiedCode === row.payment_code" class="h-3 w-3 text-emerald-500" />
                            ...
                            <td class="px-4 py-2.5">
                                <div class="flex flex-col justify-center">
                                    <div class="flex items-center gap-2 group/copy">
                                        <span class="font-bold text-slate-900 font-mono text-sm tracking-tight">{{ row.amount_label || '-' }}</span>
                                    </div>
                                    <div class="flex items-center gap-1.5 mt-1 text-slate-500">
                                        <component :is="getMethodIcon(row.method)" class="h-3.5 w-3.5 opacity-70" />
                                        <span class="text-[10px] font-medium uppercase tracking-wider">{{ row.method || '-' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-2.5 text-right pr-5">
                                <div class="flex flex-col items-end gap-1">
                                    <span class="text-[10px] text-slate-500">{{ row.created_at_label || '-' }}</span>
                                    <AppBadge :variant="statusVariant(row.status)" class="scale-90 origin-right">
                                        {{ row.status || '-' }}
                                    </AppBadge>
                                </div>
                            </td>
