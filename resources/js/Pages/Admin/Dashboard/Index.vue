<script setup>
    ...
    defineOptions({
        layout: (h, page) =>
            h(
                AdminLayout,
                { title: 'Dashboard', subtitle: 'Ringkasan aktivitas dan metrik utama.' },
                () => page
            ),
    });
    ...
    import { Boxes, CheckCircle2, ClipboardList, AlertTriangle } from 'lucide-vue-next';
    ...
    onMounted(() => {
        if (window.Echo) {
            window.Echo.channel('admin-notifications').listen('.NewRentalCreated', (e) => {
...
<template>
    ...
    <div class="h-full overflow-y-auto custom-scrollbar space-y-6 pr-1 pb-4">
        ...
        <section class="grid gap-6 xl:grid-cols-12">
            ...
            <div
                class="xl:col-span-8 flex flex-col rounded-3xl border border-slate-100 bg-white p-7 shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
            >
                <div class="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs font-medium text-slate-500 mt-1">
                            Pendapatan beberapa periode terakhir.
                        </p>
                    </div>
                </div>
            ...
            <div
                class="xl:col-span-4 flex flex-col rounded-3xl border border-slate-100 bg-white p-7 shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
            >
                <div class="mb-6">
                    <h3 class="text-base font-bold tracking-tight text-slate-900">Status Rental</h3>
                    <p class="text-xs font-medium text-slate-500 mt-1">
                        Distribusi status transaksi saat ini.
                    </p>
                </div>
        ...
        <section class="grid gap-6 xl:grid-cols-12">
            ...
            >
                <div class="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs font-medium text-slate-500 mt-1">
                            Jumlah penyewaan dalam 7 hari terakhir.
                        </p>
                    </div>
                </div>
                <div class="flex-1 min-h-[320px]">
            ...
            >
                <div class="mb-6">
                    ...
                    <p class="text-xs font-medium text-slate-500 mt-1">
                        5 produk paling banyak disewa sepanjang waktu.
                    </p>
                </div>
                <div class="flex-1 min-h-[320px]">
        ...
        <section
            class="rounded-3xl border border-slate-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden"
        >
            <div class="border-b border-slate-100/60 p-6 flex items-center justify-between">
                <div>
                    <h3 class="text-base font-bold tracking-tight text-slate-900">
                        Sirkulasi Aset Fisik & Armada
                    </h3>
                    <p class="text-xs font-medium text-slate-500 mt-1">
                        Status real-time ketersediaan dan penggunaan inventaris berdasarkan
                        kategori.
                    </p>
                </div>
            </div>
            <div class="p-6 h-[400px]">
                <FleetStatusChart
                    :labels="fleetStatusChart.labels"
                ...
                <table class="w-full text-left text-sm text-slate-600">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        >
                            ...
                            <td class="px-6 py-4 text-[13px] font-semibold text-slate-700">
                                {{ formatCurrency(trx.total_price) }}
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center rounded-full px-2.5 py-1 text-[10px] font-bold tracking-wide"
                                    :class="{
