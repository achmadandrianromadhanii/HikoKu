...
defineOptions({ layout: (h, page) => h(AdminLayout, { title: 'Kelola Kategori', subtitle: 'Atur kategori untuk pengelompokan katalog Anda.' }, () => page) })
...
watch(createAddDescription, (val) => { if (!val) createForm.description = '' })
watch(editAddDescription, (val) => { if (!val) editForm.description = '' })
...
<template>
    ...
    <div class="flex flex-col h-full gap-4">
            ...
            <div class="flex-1 min-h-0 flex flex-col rounded-3xl border border-slate-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
                ...
                <div class="shrink-0 border-b border-slate-100/60 p-4 bg-white/50 backdrop-blur-sm relative z-20">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <label class="flex items-center justify-between md:justify-start gap-3 cursor-pointer group px-2 md:px-0">
                            <span class="text-[12px] font-bold uppercase tracking-widest text-slate-500 group-hover:text-slate-700 transition-colors">Aktif Saja</span>
                            <div class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors" :class="activeOnly ? 'bg-cyan-500' : 'bg-slate-200'">
                                <input type="checkbox" v-model="activeOnly" class="sr-only" />
                                <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="activeOnly ? 'translate-x-4' : 'translate-x-1'"></span>
                            </div>
                        </label>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-50/50">
                            ...
                            <tr v-if="rows.length === 0">
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center gap-3">
                                        ...
                                        <h3 class="text-sm font-bold text-slate-700">Tidak ada kategori</h3>
                                        <p class="text-xs text-slate-500">Coba sesuaikan kata kunci atau tambah kategori baru.</p>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr v-for="row in rows" :key="row.id" class="transition-colors hover:bg-slate-100/50 group even:bg-slate-50/80">
                                ...
                                <td class="px-6 py-3 min-w-[250px]">
                                    <div class="flex items-center gap-4">
                                        <div class="relative shrink-0">
                                            <div :class="['flex h-10 w-10 items-center justify-center rounded-[14px] text-[16px] shadow-sm border border-white/50', getGradient(row.id)]">
                                                <span v-if="row.icon && row.icon.length <= 2">{{ row.icon }}</span>
                                                <span v-else class="text-[12px] font-black tracking-tight">{{ getInitials(row.name) }}</span>
                                            </div>
                                        ...
                                        <div class="flex flex-col overflow-hidden">
                                            ...
                                            <span class="text-[11px] font-medium text-slate-400 truncate mt-0.5">{{ row.description || row.slug || '-' }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-3 text-center">
                                    <span class="inline-flex items-center text-[12px] font-bold text-slate-600 bg-slate-50 px-3 py-1 rounded-full border border-slate-100">
                                        <Box class="w-3.5 h-3.5 mr-1.5 text-slate-400" />
                                        {{ row.products_count }}
                                    </span>
                                </td>
                                
                                <!-- Col 3: Sort Order -->
                                <td class="px-6 py-3 text-center tabular-nums font-bold text-[13px] text-slate-400">
                                    #{{ row.sort_order }}
                                </td>
                                
                                <!-- Col 4: Status Badges -->
                                <td class="px-6 py-3 text-center">
                                    <span v-if="row.is_active" class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-600 border border-emerald-100/50">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5 animate-pulse"></span> Aktif
                                    </span>
                                    <span v-else class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-500">
                                        Nonaktif
                                    </span>
                                </td>
                ...
                <div v-if="categories.total > 0" class="shrink-0 border-t border-slate-100/60 p-4 flex items-center justify-between bg-white/30 relative z-20">
                    <div class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                        Hal <span class="text-slate-700">{{ categories.current_page }}</span> / {{ categories.last_page }}
                    </div>
        ...
        <AppModal :open="openCreateModal" title="Tambah Kategori" max-width="max-w-md" @close="openCreateModal = false">
            <div class="space-y-4 pt-2">
                <!-- Baris 1: Nama (8/12) dan Icon (4/12) -->
                <div class="grid grid-cols-12 gap-3">
                    <div class="col-span-8">
                        <label class="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-slate-400">Nama Kategori</label>
                        ...
                        <p v-if="createForm.errors.name" class="mt-1 text-[10px] text-rose-500">{{ createForm.errors.name }}</p>
                    </div>

                    <div class="col-span-4">
                        <label class="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-slate-400">Icon / Emoji</label>
                        <input v-model="createForm.icon" type="text"
                            ...
                            placeholder="⛺, 🎒" />
                        <p v-if="createForm.errors.icon" class="mt-1 text-[10px] text-rose-500">{{ createForm.errors.icon }}</p>
                    </div>
                </div>
                ...
                <div>
                    <label class="group mb-2 flex cursor-pointer items-center gap-2">
                        <div class="relative inline-flex h-4 w-7 items-center rounded-full transition-colors" :class="createAddDescription ? 'bg-cyan-500' : 'bg-slate-200'">
                            <input type="checkbox" v-model="createAddDescription" class="sr-only" />
                            <span class="inline-block h-3 w-3 transform rounded-full bg-white transition-transform" :class="createAddDescription ? 'translate-x-3.5' : 'translate-x-0.5'"></span>
                        </div>
                        <span class="text-[11px] font-semibold text-slate-500 transition-colors group-hover:text-slate-700">Tambahkan Deskripsi (Opsional)</span>
                    </label>

                    <!-- Komentar: v-show memastikan form disembunyikan/ditampilkan tanpa menghapus dari DOM, memicu transisi jika disupport -->
                    <div v-show="createAddDescription" class="overflow-hidden transition-all duration-300">
                        <textarea v-model="createForm.description" rows="2" placeholder="Tulis deskripsi singkat..."
                            class="w-full rounded-lg border-transparent bg-slate-50 px-3 py-2 text-[12px] outline-none transition-all focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-500/20"></textarea>
                        <p v-if="createForm.errors.description" class="mt-1 text-[10px] text-rose-500">{{ createForm.errors.description }}</p>
                    </div>
                </div>
            </div>
            ...
            <template #footer>
                <div class="mt-2 flex items-center justify-between">
                    <label class="group flex cursor-pointer items-center gap-2">
                        <div class="relative inline-flex h-4 w-7 items-center rounded-full transition-colors" :class="createForm.is_active ? 'bg-cyan-500' : 'bg-slate-200'">
                            <input type="checkbox" v-model="createForm.is_active" class="sr-only" />
                            <span class="inline-block h-3 w-3 transform rounded-full bg-white transition-transform" :class="createForm.is_active ? 'translate-x-3.5' : 'translate-x-0.5'"></span>
                        </div>
                        <span class="text-[10px] font-bold uppercase tracking-widest text-slate-500 transition-colors group-hover:text-slate-700">Aktif</span>
                    </label>

                    <div class="flex items-center gap-2">
                        <button type="button" @click="openCreateModal = false"
                            class="inline-flex h-9 items-center justify-center rounded-lg border border-transparent px-3 text-[11px] font-bold text-slate-400 transition-all hover:border-slate-200 hover:bg-slate-50 hover:text-slate-600">
                            Batal
                        </button>
        ...
        <AppModal :open="openEditModal" title="Edit Kategori" max-width="max-w-md" @close="openEditModal = false">
            <div class="space-y-4 pt-2">
                <!-- Baris 1: Nama (8/12) dan Icon (4/12) -->
                <div class="grid grid-cols-12 gap-3">
                    <div class="col-span-8">
                        ...
                        <input v-model="editForm.name" type="text"
                            class="h-9 w-full rounded-lg border-transparent bg-slate-50 px-3 text-[12px] outline-none transition-all focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-500/20" />
                        <p v-if="editForm.errors.name" class="mt-1 text-[10px] text-rose-500">{{ editForm.errors.name }}</p>
                    </div>
                    ...
                    <div class="col-span-4">
                        ...
                        <input v-model="editForm.icon" type="text"
                            class="h-9 w-full rounded-lg border-transparent bg-slate-50 px-3 text-center text-[12px] outline-none transition-all focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-500/20" />
                        <p v-if="editForm.errors.icon" class="mt-1 text-[10px] text-rose-500">{{ editForm.errors.icon }}</p>
                    </div>
                </div>
                ...
                <div>
                    <label class="group mb-2 flex cursor-pointer items-center gap-2">
                        <div class="relative inline-flex h-4 w-7 items-center rounded-full transition-colors" :class="editAddDescription ? 'bg-cyan-500' : 'bg-slate-200'">
                            <input type="checkbox" v-model="editAddDescription" class="sr-only" />
                            <span class="inline-block h-3 w-3 transform rounded-full bg-white transition-transform" :class="editAddDescription ? 'translate-x-3.5' : 'translate-x-0.5'"></span>
                        </div>
                        <span class="text-[11px] font-semibold text-slate-500 transition-colors group-hover:text-slate-700">Tambahkan Deskripsi (Opsional)</span>
                    </label>

                    <!-- Komentar: v-show dipakai untuk menyembunyikan form text area yang memanjang agar terlihat super ringkas -->
                    <div v-show="editAddDescription" class="overflow-hidden transition-all duration-300">
                        <textarea v-model="editForm.description" rows="2" placeholder="Tulis deskripsi singkat..."
                            class="w-full rounded-lg border-transparent bg-slate-50 px-3 py-2 text-[12px] outline-none transition-all focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-500/20"></textarea>
                        <p v-if="editForm.errors.description" class="mt-1 text-[10px] text-rose-500">{{ editForm.errors.description }}</p>
                    </div>
                </div>
            </div>
            ...
            <template #footer>
                <div class="mt-2 flex items-center justify-between">
                    <label class="group flex cursor-pointer items-center gap-2">
                        <div class="relative inline-flex h-4 w-7 items-center rounded-full transition-colors" :class="editForm.is_active ? 'bg-cyan-500' : 'bg-slate-200'">
                            <input type="checkbox" v-model="editForm.is_active" class="sr-only" />
                            <span class="inline-block h-3 w-3 transform rounded-full bg-white transition-transform" :class="editForm.is_active ? 'translate-x-3.5' : 'translate-x-0.5'"></span>
                        </div>
                        <span class="text-[10px] font-bold uppercase tracking-widest text-slate-500 transition-colors group-hover:text-slate-700">Aktif</span>
                    </label>
        ...
        <ConfirmModal :open="openDeleteModal" title="Hapus Kategori" message="Yakin ingin menghapus kategori ini secara permanen?"
            confirm-text="Ya, Hapus" :loading="actionLoading" variant="danger" @close="openDeleteModal = false"
