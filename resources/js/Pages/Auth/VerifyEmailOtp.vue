...
import { Mail, ArrowRight, LoaderCircle, ShieldCheck, AlertCircle, RefreshCw } from 'lucide-vue-next'
...
<template>
    ...
    <AuthLayout title="Verifikasi Kode OTP">
        ...
        <div class="fixed inset-0 z-[-1] overflow-hidden bg-slate-900 pointer-events-none">
            ...
            <img src="https://images.unsplash.com/photo-1506744626753-1fa44df14d28?q=80&w=1920&fm=webp" 
                 alt="Background" 
                 class="absolute inset-0 h-full w-full object-cover opacity-30" />
        </div>

        <div class="mx-auto flex min-h-[calc(100vh-84px)] w-full max-w-[480px] items-center justify-center px-4 py-8">
            <section class="w-full rounded-[28px] border border-white/20 bg-white/90 backdrop-blur-xl p-8 shadow-[0_30px_60px_rgba(0,0,0,0.5)] transition-all duration-500"
                     :class="{'ring-4 ring-emerald-400/50 bg-emerald-50/90': isSuccess}">
                
                <div class="text-center mb-8">
                    <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 text-blue-600 shadow-inner"
                ...
                <div v-if="isSuccess" class="py-8 text-center text-emerald-600 font-bold animate-pulse">
                ...
                <div v-else>
                    ...
                    <div class="mb-6 flex justify-center gap-2 sm:gap-3">
                        <input 
                            ...
                            :class="{'border-red-300 bg-red-50': page.props.errors.otp || verifyForm.errors.code}"
                        />
                    </div>

                    <!-- Hitung Mundur & Kirim Ulang -->
                    <div class="flex items-center justify-between text-sm font-medium mt-6">
                        <div class="flex items-center gap-2" :class="isTimerRunning ? 'text-blue-600' : 'text-slate-500'">
                        ...
                        >
                            <RefreshCw class="h-3.5 w-3.5" :class="{'animate-spin': sendForm.processing}" />
                            <span>Kirim Ulang Kode</span>
                        </button>
                    </div>

                    <div v-if="verifyForm.processing" class="mt-8 text-center text-sm font-semibold text-blue-600 animate-pulse">
                        Memverifikasi kode...
                    </div>
                </div>
            </section>
