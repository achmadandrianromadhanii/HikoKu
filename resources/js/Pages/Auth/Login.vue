...
onMounted(() => {
    ...
    if (isLocalhost && isHttp) {
        ...
        if (turnstileContainer.value) {
            turnstileContainer.value.innerHTML = '<div class="text-[10px] text-slate-400 text-center py-2 italic font-mono">Turnstile Bypassed (Local Mode)</div>';
        }
    ...
    const renderTurnstile = () => {
        if (window.turnstile && turnstileContainer.value) {
            ...
            turnstileWidgetId = window.turnstile.render(turnstileContainer.value, {
                ...
                sitekey: window.location.protocol !== 'https:' 
...
<template>
    ...
    <AuthLayout title="Form Login">
        ...
        <transition enter-active-class="transition duration-500 ease-out" enter-from-class="translate-x-12 opacity-0" enter-to-class="translate-x-0 opacity-100" leave-active-class="transition duration-300 ease-in" leave-from-class="translate-x-0 opacity-100" leave-to-class="translate-x-12 opacity-0">
            <div v-if="showSuccessToast" class="fixed right-6 top-6 z-50 flex items-center gap-3 rounded-2xl border border-emerald-400/20 bg-gradient-to-r from-emerald-500 to-emerald-600 px-5 py-4 text-white shadow-[0_10px_40px_rgba(16,185,129,0.3)] backdrop-blur-md">
        ...
        <div class="mx-auto flex min-h-[100dvh] w-full max-w-[420px] items-center justify-center px-4 py-2 sm:px-6 lg:px-8">
            ...
            <section class="w-full rounded-[32px] border border-white/10 bg-[#0B1120]/90 backdrop-blur-md md:backdrop-blur-2xl px-5 py-5 shadow-[0_20px_60px_rgba(0,0,0,0.6)] sm:px-8 sm:py-8 relative overflow-hidden">
                ...
                <div class="absolute -top-16 -left-16 h-40 w-40 rounded-full pointer-events-none" style="background: radial-gradient(circle, rgba(6, 182, 212, 0.2) 0%, transparent 70%);"></div>
                <div class="absolute -bottom-16 -right-16 h-40 w-40 rounded-full pointer-events-none" style="background: radial-gradient(circle, rgba(16, 185, 129, 0.1) 0%, transparent 70%);"></div>
                ...
                <div class="mb-4 flex flex-col items-center justify-center relative z-10">
                    ...
                    <p class="text-[11px] font-bold text-cyan-400 tracking-widest uppercase mt-2">Welcome Back</p>
                </div>
                ...
                <form class="space-y-3 relative z-10" @submit.prevent="submit">
                        ...
                        <div>
                            <div class="group relative z-0 w-full">
                                ...
                                <span class="pointer-events-none absolute left-3.5 top-2.5 text-slate-500 transition-colors peer-focus:text-cyan-400">
                                    <Mail class="h-5 w-5" />
                                </span>
                            ...
                            <p v-if="form.errors.email" class="mt-1 pl-1 text-[11px] font-bold text-rose-400">
                                {{ form.errors.email }}
                            </p>
                        </div>
                        ...
                        <div>
                            <div class="group relative z-0 w-full">
                                <!-- [PENJELASAN KODE UPDATE]: Mengurangi padding dalam (py-3.5 jadi py-2.5) agar lebih ringkas -->
                                <input v-model="form.password" :type="showPassword ? 'text' : 'password'" id="password" autocomplete="current-password" placeholder=" "
                            ...
                            <p v-if="form.errors.password" class="mt-1.5 pl-1 text-[11px] font-bold text-rose-400">
                                {{ form.errors.password }}
                            </p>
                        </div>

                        <!-- Opsi Ingat Saya & Loader (Glassmorphism Pill) -->
                        <div class="flex items-center justify-between gap-3 rounded-full bg-white/5 border border-white/5 px-4 py-2 w-full sm:w-2/3 max-w-[240px]">
                            <label class="flex cursor-pointer items-center gap-2 text-[12px] font-bold text-slate-300 transition hover:text-cyan-400">
                                <input v-model="form.remember" type="checkbox"
                                    class="rounded bg-black/50 border-white/20 text-cyan-500 transition focus:ring-cyan-500/50" />
                                Ingat Saya
                            </label>

                            <div class="inline-flex items-center gap-1.5 text-[11px] font-bold text-cyan-400" v-if="form.processing">
                        ...
                        <div class="flex flex-col items-center justify-center py-1">
                            <div ref="turnstileContainer" class="transform scale-90 origin-center bg-white/5 rounded-xl border border-white/10 p-1 overflow-hidden shadow-inner"></div>
                            <p v-if="form.errors['cf-turnstile-response']" class="mt-1 text-center text-[11px] font-bold text-rose-400">
                        ...
                        <div class="flex items-center justify-center gap-4 pt-0">
                            ...
                            <a v-if="googleLoginUrl" :href="googleLoginUrl"
                                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 shadow-lg backdrop-blur-md transition-all hover:scale-110 hover:border-white/20 hover:bg-white/10 active:scale-95" title="Login with Google">
                            ...
                            <!-- GitHub -->
                            <a v-if="githubLoginUrl" :href="githubLoginUrl"
                                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 shadow-lg backdrop-blur-md transition-all hover:scale-110 hover:border-white/20 hover:bg-white/10 active:scale-95" title="Login with GitHub">
                                <svg class="h-6 w-6 shrink-0" fill="#ffffff" viewBox="0 0 24 24" aria-hidden="true">
                            ...
                            <!-- Discord -->
                            <a v-if="discordLoginUrl" :href="discordLoginUrl"
                                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 shadow-lg backdrop-blur-md transition-all hover:scale-110 hover:border-[#5865F2]/50 hover:bg-[#5865F2]/20 active:scale-95" title="Login with Discord">
                                <svg class="h-6 w-6 shrink-0" fill="#5865F2" viewBox="0 0 127.14 96.36" aria-hidden="true">
                        ...
                        <div class="mt-4 flex flex-col items-center space-y-1 pt-1">
                            <p class="text-[12px] font-bold text-slate-400">
                                Belum ada akun?
                                <Link :href="route('register')" class="ml-1 font-black text-cyan-400 transition hover:text-cyan-300 hover:underline">
                                    Daftar Sekarang
                                </Link>
                            </p>
