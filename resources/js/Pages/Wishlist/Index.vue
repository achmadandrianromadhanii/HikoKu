...
<template>
    ...
    <DefaultLayout>
        ...
        <section class="mx-auto max-w-7xl px-4 pt-4 lg:pt-16 pb-6 sm:px-6 lg:px-8">
            ...
            <div class="lg:hidden bg-slate-50 dark:bg-[#0B1120]/40 -mx-4 px-4 py-4 mb-6 shadow-sm dark:shadow-lg border-b border-slate-200 dark:border-white/10 flex items-center justify-between relative z-10 backdrop-blur-2xl">
                <div class="flex items-center gap-3">
                    <button type="button" @click="() => window.history.length > 1 ? window.history.back() : router.visit(route('home'))" class="flex h-9 w-9 items-center justify-center rounded-full bg-slate-200 dark:bg-white/5 border border-slate-300 dark:border-white/10 text-slate-700 dark:text-slate-300 active:scale-90 transition-transform">
                        <ChevronLeft class="h-5 w-5" />
                    </button>
                    <div>
                        <h1 class="text-[15px] font-black text-slate-800 dark:text-white tracking-tight">Wishlist Saya</h1>
                        <p class="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 mt-0.5">{{ products.length }} Barang Tersimpan</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4 hidden lg:flex justify-start">
                <span class="inline-flex items-center rounded-full bg-cyan-50/50 px-4 py-1 text-[13px] tracking-[0.2em] text-cyan-700 shadow-sm ring-1 ring-inset ring-cyan-600/10 backdrop-blur-sm font-serif italic">
                    Wishlist
                </span>
            </div>
            ...
            <div v-else class="flex min-h-[50vh] items-center justify-center">
                <div class="group relative flex flex-col items-center justify-center overflow-hidden rounded-[32px] border border-white/40 bg-white/30 px-6 py-16 text-center shadow-xl backdrop-blur-xl sm:px-12 transition-all duration-700 hover:bg-white/40 hover:shadow-2xl">
                    <!-- Glow Latar Belakang -->
                    <div class="absolute inset-0 -z-10 bg-gradient-to-br from-cyan-100/40 to-emerald-100/40 opacity-50 blur-3xl transition-opacity duration-700 group-hover:opacity-100"></div>
                    
                    <!-- Ikon Premium dengan Animasi Goyang (Pulse/Bounce) -->
                    <div class="relative mb-6 flex h-24 w-24 items-center justify-center rounded-[24px] bg-gradient-to-tr from-cyan-50 to-emerald-50 shadow-sm border border-white/60 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3">
                        <PackageSearch class="h-10 w-10 text-cyan-600 drop-shadow-sm transition-transform duration-500 group-hover:text-cyan-500" />
                        <div class="absolute -right-1 -top-1 h-6 w-6 animate-ping rounded-full bg-teal-400 opacity-20"></div>
                    </div>
                    
                    <h2 class="mb-3 text-[22px] font-extrabold tracking-tight text-slate-800">Belum ada favorit</h2>
                    <p class="mb-8 max-w-[280px] text-[13px] font-medium leading-relaxed text-slate-500">
                        Koleksi produk impianmu masih kosong. Yuk jelajahi katalog kami dan simpan peralatan outdoor terbaik untuk petualanganmu!
                    </p>
